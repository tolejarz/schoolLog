...
function BytesUploaded(
	...
	__filemonitor.onLoad = function(s) {
		...
		if(s && this.filesize && this.filesize != 'undefined')
			{
				whatsup = __fSize(this.filesize) + ' envoyés...';
				$(function() {$("#progressbar").progressbar('option', 'value', ($("#progressbar").progressbar('option', 'value') + 10)%100)});
			}
		else if(this.filesize && this.filesize == 'undefined') {
			whatsup = 'Attente de la réponse du fichier temporaire ...';
			if(__maybesomethingwrong++ > 10) {
				__fileloaderInterval = 0;
				whatsup = 'Dossier temporaire invalide.';
			}
		}
		else {
			whatsup = 'Impossible de trouver les informations serveur.';
			__fileloaderInterval = 0;
		}
