...
if (empty($parms['subjects'])) {
	...
	foreach ($parms['subjects'] as $subject) {
		echo('
			<tr>
				<td class="centered">
					<a href="' . Router::build('ClassSubjectDelete', array('class_id' => $parms['id_classe']), array('id_matiere' => $subject['id'])) . '" target="_self"><img alt="Supprimer" src="/img/delete.png" title="Supprimer" /></a>
					<a href="' . Router::build('ClassSubjectEdit', array('class_id' => $parms['id_classe']), array('id_matiere' => $subject['id'])) . '" target="_self"><img alt="Modifier" src="/img/edit.png" title="Modifier" /></a></td>
				<td>' . $subject['nom'] . '</td>
				<td>');
