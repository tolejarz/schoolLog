	...
	function _doDelete() {
		if ($_SESSION['user_privileges'] == 'enseignant' || $_SESSION['user_privileges'] == 'superviseur') {
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				$this->redirect('SupportList'); // ajouter le retour à la bonne matière/classe pour le superviseur
			} else {
				/* Récupération des informations associées au support dans la base */
				import('SupportDeleteView');
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if (!isset($_SESSION['ERROR_MSG'])) {
					if (isset($_POST['validation'])) {
						...
						$m->update($this->_getArg('id'), $params);
					}
					$this->redirect('SupportList');
				}
			}
