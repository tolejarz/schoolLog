...
class CalendrierController extends Controller {
	function _doDefaultAction() {
		import('CalendrierModel');
		import('CalendrierDefaultView');
	...
	function _doVoirEnseignants() {
		if (in_array($_SESSION['user_privileges'], array('enseignant', 'superviseur'))) {
			import('CalendrierEnseignantsSelectView');
	...
	function _doAddDemande() {
		if (in_array($_SESSION['user_privileges'], array('enseignant', 'superviseur'))) {
			...
			import('ClasseModel');
			import('CalendrierDemandeAddView');
	...
	function _doRejectDemande() {
		if (in_array($_SESSION['user_privileges'], array('enseignant', 'superviseur'))) {
			import('CalendrierDemandeRejectView');
			import('CalendrierModel');
			
			$id = $this->_getArg('id');
			
			$m = new CalendrierModel($this->dbo);
			$r = $m->getOperation(array('id' => $id));
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if ($m->updateOperation($id, array('etat' => 'refusée')) > 0) {
	...
	function _doAcceptDemande() {
		if (in_array($_SESSION['user_privileges'], array('enseignant', 'superviseur'))) {
			import('CalendrierDemandeAcceptView');
			import('CalendrierModel');
			
			$ajax = $this->_getArg('ajax');
			
			$m = new CalendrierModel($this->dbo);
			$r = $m->getOperation(array('id' => $this->_getArg('id')));
			...
			if ($_SERVER['REQUEST_METHOD'] == 'POST') {
				...
				if (empty($_POST['date_report']) && empty($ajax)) {
					...
					if (empty($ajax)) {
						...
						$wawa = $this->_getArg('id');
					if ($m->updateOperation($this->_getArg('id'), $params) > 0) {
						$m = new CalendrierModel($this->dbo);
						$r = $m->getOperation(array('id' => $this->_getArg('id')));
	...
	function _doHistory() {
		if (in_array($_SESSION['user_privileges'], array('enseignant', 'superviseur'))) {
			...
			import('CalendrierDemandeHistoryView');
			$params = array(
	...
	function _doShowPeriods() {
		...
		import('ClasseModel');
		import('CalendrierModel');
		import('CalendrierShowPeriodesView');
	...
	function _doDeletePeriod() {
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			...
			$this->redirect('CalendarPeriodList', array('id_classe' => $r['id_classe']));
		}
		import('CalendrierDeletePeriodView');
		$v = new CalendrierDeletePeriodView();
		$v->show($r);
	}
	
	function _doAddPeriod() {
		// Si l'utilisateur n'est pas superviseur on interrompt le script
		if ($_SESSION['user_privileges'] != 'superviseur') {
			die();
		}
		
		$id = $this->_getArg('id_classe');
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			if (!isset($_POST['validation'])) {
				$this->redirect('CalendarPeriodList', array('id_classe' => $id));
			}
		...
		import('CalendrierAddPeriodView');
					...
					foreach ($_POST as $k => $v) {
						if (substr($k, 0, 5) == 'event') {
							...
							if (count($tmp) == 5) {
								...
								$usedIds[] = $this->dbo->insert('insert into modele_planning(jour, heure_debut, heure_fin, id_matiere, id_enseignant, id_classe, id_periode) values(' . intval($tmp[0] + 1) . ', "' . $this->FormatTimeFrToUs($tmp[1]) . '", "' . $this->FormatTimeFrToUs($tmp[2]) . '", ' . intval($tmp[3]) . ', (select id_enseignant from enseignants_matieres_classes where id_matiere=' . intval($tmp[3]) . ' and id_classe=' . $r['id_classe'] . ' limit 0, 1), ' . $r['id_classe'] . ', ' . $id . ')');
							}
						}
					}
					$this->dbo->delete('delete from modele_planning where id not in (' . implode(',', $usedIds) . ') and id_periode=' . $id);
				}
			}
			$this->redirect('CalendarPeriodList', array('id_classe' => $r['id_classe']));
		}
		import('CalendrierEditPeriodView');
		import('ClasseModel');
		import('MatieresClasseModel');
		$c = new ClasseModel($this->dbo);
		$v = new CalendrierEditPeriodView();
		$r['classes'] = $c->listing();
		$r['cours'] = $this->dbo->query('select id, jour+0 as jour, jour as jour_libelle, date_format(heure_debut, "%H:%i") as heure_debut, date_format(heure_fin, "%H:%i") as heure_fin, id_matiere, id_periode from modele_planning where id_classe=' . $r['id_classe'] . ' and id_periode=' . $id);
		$m = new MatieresClasseModel($this->dbo);
